...
import pika
import time
from alignment_fastq import perform_alignment_fastq, RABBITMQ_HOST, ALIGNMENT_FASTQ_QUEUE
import logging
...
def process_message(message):
    try:
        fastq_files = message["fastq_files"]
        reference_file = message["reference_file"]
        is_paired = message["is_paired"]
        ...
        perform_alignment_fastq(fastq_files, reference_file, is_paired, user_id)
...
def main():
    ...
    for attempt in range(max_retries):
        try:
            ...
            channel.queue_declare(queue=ALIGNMENT_FASTQ_QUEUE, durable=True)

            # Start consuming from the queue using the default exchange
            channel.basic_consume(
                queue=ALIGNMENT_FASTQ_QUEUE, on_message_callback=callback, auto_ack=True
            )
